...
import {
  Activity, Archive, Box, Check, ChevronLeft, CloudOff, Database,
  Download, FileJson, FileSpreadsheet, Gauge, HardDrive, Info, LineChart as LineIcon, LockKeyhole,
  Minus, Moon, PackagePlus, Pencil, Plus, Search, Settings, ShieldCheck, ShoppingBasket,
...
import type { BackupData, Product, Purchase, Store, ThemeMode } from './lib/types'
import { buildCompositeIndex, categoryInflation, filterPurchases, getProductStats, lastNDaysISO, periodProductChange, sortPurchases, storeComparison, totalSpend } from './lib/analytics'
import { downloadText, encryptBackup, parseBackup, purchasesToCsv } from './lib/backup'
...
import { ProductForm, PurchaseForm } from './components/forms'
...
function monthStartISO() { const d = new Date(); d.setDate(1); const offset = d.getTimezoneOffset()*60_000; return new Date(d.getTime()-offset).toISOString().slice(0,10) }
export default function App() {
  ...
  const loading = productsLive === undefined || storesLive === undefined || purchasesLive === undefined
  ...
  }, [purchases.length, products.length])
  ...
  const monthSpend = useMemo(() => totalSpend(filterPurchases(purchases, monthStartISO(), todayISO())), [purchases])
  ...
  const backupDue = purchases.length >= 10 && (backupAgeDays == null || backupAgeDays >= BACKUP_REMINDER_DAYS)
  ...
  function backupObject(exportedAt = new Date().toISOString()): BackupData { return { schema:'gheymat-negar', version:1, exportedAt, products, stores, purchases, settings: { ...settings, lastBackupAt: exportedAt } } }
  ...
  function csvExport() { downloadText(`gheymat-negar-purchases-${todayISO()}.csv`, purchasesToCsv(backupObject()), 'text/csv;charset=utf-8'); pushToast('CSV خریدها ساخته شد.') }
  ...
  async function importFile(file: File) {
    try {
      ...
      const fileSummary = `${faNumber.format(data.products.length)} کالا، ${faNumber.format(data.purchases.length)} خرید و ${faNumber.format(data.stores.length)} فروشگاه`
      if (importMode === 'replace') {
        ...
        await db.transaction('rw', [db.products, db.stores, db.purchases, db.settings], async () => {
          await Promise.all([db.products.clear(), db.stores.clear(), db.purchases.clear(), db.settings.clear()])
          await db.products.bulkAdd(data.products); await db.stores.bulkAdd(data.stores); await db.purchases.bulkAdd(data.purchases); await db.settings.put(data.settings)
        })
      } else {
        const { adoptIncomingCurrency } = resolveMergeCurrency(settings.currency, data.settings.currency, purchases.length, data.purchases.length)
        const plan = planBackupMerge({ products, stores, purchases }, data)
        const dedupeNote = plan.dedupedProducts || plan.dedupedStores
          ...
          : ''
        const currencyNote = adoptIncomingCurrency ? ` چون هنوز خریدی در داده فعلی نیست، واحد پول «${data.settings.currency}» از پشتیبان پذیرفته می‌شود.` : ''
  ...
  async function clearAll() {
    if (!await askConfirm('پاک کردن همه داده‌ها', 'تمام کالاها، فروشگاه‌ها و خریدها برای همیشه از این مرورگر حذف می‌شوند و این عمل برگشت‌پذیر نیست.', 'همه را پاک کن', true)) return
    await db.transaction('rw', [db.products, db.stores, db.purchases, db.settings], async()=>{ await db.purchases.clear(); await db.stores.clear(); await db.products.clear(); await db.settings.put({ ...settings, lastBackupAt: undefined }) })
    pushToast('همه داده‌ها پاک شدند.', 'info')
  }

  async function loadDemo() {
    if (products.length || purchases.length || stores.length) return pushToast('داده نمونه فقط روی دیتابیس خالی اضافه می‌شود. ابتدا پشتیبان بگیرید و داده‌ها را پاک کنید.', 'info')
    const demo = makeDemoData(); await db.products.bulkAdd(demo.products); await db.stores.bulkAdd(demo.stores); await db.purchases.bulkAdd(demo.purchases); pushToast('داده نمونه اضافه شد.')
  }
  ...
  const renderDashboard = () => <>
    <div className="page-heading">
      <div>
        ...
        <h1>داشبورد قیمت‌ها</h1>
        <p>روند قیمت خریدهای واقعی شما؛ بدون ارسال داده‌های ثبت‌شده به سرور.</p>
      </div>
      <button className="button primary page-heading-action" onClick={() => setPurchaseModal({ open: true })} disabled={!activeProducts.length}>
        <Plus size={19}/>ثبت خرید
      </button>
    </div>

    {!products.length ? <EmptyState
      icon={<ShoppingBasket size={28}/>}
      title="از اولین کالا شروع کنید"
      body="یک کالا بسازید، بعد هر بار که آن را می‌خرید قیمت را ثبت کنید. نمودارها به‌تدریج دقیق‌تر می‌شوند."
      action={<div className="button-row"><button className="button primary" onClick={() => setProductModal({ open: true })}><PackagePlus size={18}/>افزودن کالا</button><button className="button secondary" onClick={loadDemo}>نمایش داده نمونه</button></div>}
    /> : <>
      {!activeProducts.length ? <section className="onboarding-callout">
      ...
      <div className="stats-grid">
        ...
        <StatCard label="هزینه ثبت‌شده این ماه" value={money(monthSpend)} hint={`${faNumber.format(filterPurchases(purchases, monthStartISO(), todayISO()).length)} خرید`}/>
        <StatCard label="هزینه ۳۰ روز اخیر" value={money(last30Spend)} hint="جمع مبلغ خریدهای ثبت‌شده"/>
  ...
  const renderData = () => <>
    ...
    <div className="settings-grid">
      ...
      <section className="panel">
        ...
        <label className="field"><span>واحد پول</span><input key={settings.currency} defaultValue={settings.currency} disabled={purchases.length > 0} onBlur={event => db.settings.put({ ...settings, currency: event.target.value.trim() || 'تومان' })} placeholder="تومان"/><small className="field-help">{purchases.length ? 'برای جلوگیری از برچسب‌گذاری اشتباه مبالغ قبلی، واحد پول بعد از اولین خرید قفل می‌شود.' : 'این گزینه واحد نمایش همه مبالغ است؛ قبل از شروع ثبت خرید آن را مشخص کنید.'}</small></label>
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className="page-content">{page==='dashboard'?renderDashboard():page==='products'?renderProducts():page==='analytics'?renderAnalytics():renderData()}</div>
    </main>
